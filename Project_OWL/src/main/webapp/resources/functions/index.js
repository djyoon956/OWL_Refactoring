...
exports.sendNotification = functions.database.ref('Messages/{roomId}/{messageId}').onCreate(event => { 
	...
	const promiseRoomUserList = admin.database().ref(`UsersInRoom/${roomId}`).once('value'); // 채팅방 유저리스트 
	...
	return Promise.all([promiseRoomUserList, promiseUsersConnection]).then(results => { 
		...
		if(usersConnectionSnapShot.hasChildren()){ 
			usersConnectionSnapShot.forEach(snapshot => { 
				...
				if(value){ arrConnectionUserList.push(snapshot.key); 
				} 
				}) 
				}else{ 
				...
				if(token){ 
					const payload = { 
							notification: { 
								title: sendUserName, 
								body: sendMsg, 
								click_action :`https://owl-chat-c27f1.firebaseio.com/?roomId=${roomId}`, 
								// icon: sendProfile 
								} 
					}; 
